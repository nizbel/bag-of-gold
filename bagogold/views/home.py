# -*- coding: utf-8 -*-
from bagogold.models.acoes import OperacaoAcao, HistoricoAcao, Provento
from bagogold.models.fii import OperacaoFII, HistoricoFII, ProventoFII
...
from bagogold.utils.lc import calcular_valor_lc_ate_dia
...
def home(request):
    ...
    for item in lista_conjunta:      
        ...
        if item.data != datetime.date.today():
            for papel in fii.keys():
                ...
                while not HistoricoFII.objects.filter(data=ultimo_dia_util, fii__ticker=papel):
                    ultimo_dia_util -= datetime.timedelta(days=1)
                patrimonio['FII'] += (fii[papel] * HistoricoFII.objects.get(fii__ticker=papel, data=ultimo_dia_util).preco_unitario)
        else:
            for papel in fii.keys():
                patrimonio['FII'] += (fii[papel] * Decimal(Share('%s.SA' % (papel)).get_price()))
        ...
        while not HistoricoTaxaDI.objects.filter(data=ultimo_dia_util):
                ultimo_dia_util -= datetime.timedelta(days=1)
        patrimonio['Letras de Crédito'] = calcular_valor_lc_ate_dia(ultimo_dia_util)
