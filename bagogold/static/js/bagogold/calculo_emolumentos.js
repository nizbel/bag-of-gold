$(document).ready(function() {
    ...
    $('#id_preco_unitario').change(function() {
        var preco = $('#id_preco_unitario').val().replace(/\./g, '').replace(/,/g, '.');
        
        var selected = $("input[type='radio'][name='radioDT']:checked");
        if (selected.length > 0 && selected.val() == "1") {
            var emolumentos = (preco * $('#id_quantidade').val().replace(/\./g, '')) * 0.025 / 100;
        } else {
            var emolumentos = (preco * $('#id_quantidade').val().replace(/\./g, '')) * 0.0325 / 100;
        }
