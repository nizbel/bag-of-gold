$(document).ready(function() {
	form_count = $("input[name=divisaooperacaoletracambio_set-TOTAL_FORMS]").val();
    // get extra form count so we know what index to use for the next item.
     $("#add-another").click(function() {
         ...
         form_count ++;
         $("[name=divisaooperacaoletracambio_set-TOTAL_FORMS]").val(form_count);
     ...
     if ($('#id_tipo_operacao').parent().find('button').attr('title') == 'Venda') {
        ...
        $("#grupo_lc").addClass('hidden');
    } else {
        $("#grupo_operacao_compra").addClass('hidden');
        $("#grupo_lc").removeClass('hidden');
    }
