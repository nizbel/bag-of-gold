...
def calcular_rendimentos_ate_data(investidor, data, tipo_investimentos='BCDFILT'):
    ...
    if 'C' in tipo_investimentos:
        rendimentos['C'] = sum(calcular_valor_cdb_rdb_ate_dia(investidor, data).values()) \
            - sum([(operacao.quantidade * operacao.preco_unitario) for operacao in OperacaoCDB_RDB.objects.filter(data__lte=data, tipo_operacao='C')]) \
            + sum([(operacao.quantidade * operacao.preco_unitario) for operacao in OperacaoCDB_RDB.objects.filter(data__lte=data, tipo_operacao='V')])
