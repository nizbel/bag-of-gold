...
def calcular_rendimentos_ate_data(investidor, data, tipo_investimentos='BCDEFILRT'):
    ...
    if 'R' in tipo_investimentos:
        rendimentos['R'] = sum(calcular_valor_cri_cra_ate_dia(investidor, data).values()) \
            - sum([(operacao.quantidade * operacao.preco_unitario) for operacao in OperacaoCRI_CRA.objects.filter(investidor=investidor, data__lte=data, tipo_operacao='C')]) \
            + sum([(operacao.quantidade * operacao.preco_unitario) for operacao in OperacaoCRI_CRA.objects.filter(investidor=investidor, data__lte=data, tipo_operacao='V')])
