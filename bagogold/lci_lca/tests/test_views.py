...
class EditarOperacaoLetraCreditoTestCase(TestCase):
    def setUp(self):
        ...
        lci = LetraCredito.objects.create(investidor=investidor, nome='LCI', tipo_rendimento=LetraCredito.LCI_LCA_DI)
        ...
        DivisaoOperacaoLetraCredito.objects.create(operacao=compra_1, divisao=Divisao.objects.get(investidor=nizbel.investidor), quantidade=compra_1.quantidade)
        ...
        compra_2 = OperacaoLetraCredito.objects.create(quantidade=1000, data=datetime.date(2018, 15, 4), 
                                                      tipo_operacao='C', letra_credito=lci, investidor=multi_div.investidor)
        div_geral_compra_2 = DivisaoOperacaoCriptomoeda.objects.create(operacao=compra_2, divisao=div_geral, quantidade=600)
        nova_div_compra_2 = DivisaoOperacaoCriptomoeda.objects.create(operacao=compra_2, divisao=nova_div, quantidade=400)
    ...
    def test_editar_operacao_sucesso_1_div(self):
        ...
        self.assertFalse(DivisaoOperacaoLetraCredito.objects.filter(divisao=Divisao.objects.get(investidor=investidor), quantidade=1100, 
        ...
        self.assertTrue(OperacaoLetraCredito.objects.filter(quantidade=1100, investidor=investidor,
                                                              data=datetime.date(2018, 4, 12)).exists())
        self.assertTrue(DivisaoOperacaoLetraCredito.objects.filter(divisao=Divisao.objects.get(investidor=investidor), quantidade=1100, 
                                                              operacao__id=operacao.id).exists())
    ...
    def test_excluir_operacao_1_div_sucesso(self):
        ...
        self.assertTrue(DivisaoOperacaoLetraCredito.objects.filter(divisao=Divisao.objects.get(investidor=investidor), 
        ...
        self.assertFalse(DivisaoOperacaoLetraCredito.objects.filter(divisao=Divisao.objects.get(investidor=investidor), 
                                                              operacao__id=operacao_id).exists())
    ...
    def test_excluir_operacao_multi_div_sucesso(self):
        ...
        self.assertTrue(OperacaoLetraCredito.objects.filter(id=operacao_id, investidor=investidor).exists())
        self.assertTrue(DivisaoOperacaoLetraCredito.objects.filter(operacao__id=operacao_id).count() > 0)
        ...
        self.assertEqual(response.url, reverse('lci_lca:historico_lci_lca'))
        self.assertFalse(OperacaoLetraCredito.objects.filter(id=operacao_id, investidor=investidor).exists())
        self.assertFalse(DivisaoOperacaoLetraCredito.objects.filter(operacao__id=operacao_id).exists())
    ...
    def test_nao_permitir_excluir_operacao_com_venda(self):
        ...
        self.cliente.login(username='nizbel', password='nizbel')
        ...
        response = self.client.post(reserver('lci_lca:editar_operacao_lci_lca', kwargs={'operacao_id': operacao.id,
            'delete': 1,
        })
        ...
        self.assertTrue(OperacaoLetraCredito.objects.filter(investidor=investidor, id=operacao_id).exists())